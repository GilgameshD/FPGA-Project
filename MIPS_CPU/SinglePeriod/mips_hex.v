			...
			8'h01:    Instruction <= 32'h08000059;
				//j       Exception     # exception entrance
			8'h02:    Instruction <= 32'h08000067;
				//jal     KernelToUser
			...
			8'h44:    Instruction <= 32'h3c08ffff;
				//addi    $t0,     $t0,     0x0000 # low 4 bits
			8'h45:    Instruction <= 32'h21080000;
				//sw      $zero,   8($s7)          # TCON = 0
			8'h46:    Instruction <= 32'haee00008;
				//sw      $t0,     0($s7)          # TH
			8'h47:    Instruction <= 32'haee80000;
				//sw      $t0,     4($s7)          # TL
			8'h48:    Instruction <= 32'haee80004;
				//addi    $t0,     $zero,   3
			8'h49:    Instruction <= 32'h20080003;
				//sw      $t0,     8($s7)          # TCON = 3
			8'h4a:    Instruction <= 32'haee80008;
				//beq     $s1,     $zero,       ZERO
			8'h4b:    Instruction <= 32'h1220000a;
				//beq     $s0,     $zero,       ZERO
			8'h4c:    Instruction <= 32'h12000009;
				//slt     $s2,     $s0,         $s1
			8'h4d:    Instruction <= 32'h0211902a;
				//beq     $s2,     $zero,       S0_BIGGER
			8'h4e:    Instruction <= 32'h12400002;
				//sub     $s1,     $s1,         $s0
			8'h4f:    Instruction <= 32'h02308822;
				//j       Calculate
			8'h50:    Instruction <= 32'h0800004b;
				//sub     $s0,     $s0,         $s1
			8'h51:    Instruction <= 32'h02118022;
				//bne     $s0,     $zero,       Calculate   # not zero
			8'h52:    Instruction <= 32'h1600fff8;
				//sw      $s1,     0x18($s7)                # send UART
			8'h53:    Instruction <= 32'haef10018;
				//sw      $s1,     0x0c($s7)
			8'h54:    Instruction <= 32'haef1000c;
				//j       UARTWait
			8'h55:    Instruction <= 32'h08000031;
				//sw      $zero,   0x18($s7)            # send UART
			8'h56:    Instruction <= 32'haee00018;
				//sw      $zero,   0x0c($s7)
			8'h57:    Instruction <= 32'haee0000c;
				//j       UARTWait  # wait next UART value
			8'h58:    Instruction <= 32'h08000031;
				//lw      $t4,     0x50($zero)          # read the digital scan choose bit
			8'h59:    Instruction <= 32'h8c0c0050;
				//lw      $t5,     0x40($t4)            # read UART data (low 4 bits) (max is f)
			8'h5a:    Instruction <= 32'h8d8d0040;
				//sll     $t5,     $t5,          2      # multiple by 4 (get its position)
			8'h5b:    Instruction <= 32'h000d6880;
				//lw      $t6,     0x00($t5)            # get the number (low 8 bits)
			8'h5c:    Instruction <= 32'h8dae0000;
				//sll     $zero,   $zero,        0      # a nop to solve load-use 2 type
			8'h5d:    Instruction <= 32'h00000000;
				//lw      $t3,     0x54($t4)            # get the digital scan choose number (high 4 bits)
			8'h5e:    Instruction <= 32'h8d8b0054;
				//add     $t2,     $t3, $t6             # add two number to get 12bits (port: digi)
			8'h5f:    Instruction <= 32'h016e5020;
				//sw      $t2,     0x14($s7)            # write RAM and display the number
			8'h60:    Instruction <= 32'haeea0014;
				//addi    $t3,     $zero,        3
			8'h61:    Instruction <= 32'h200b0003;
				//sw      $t3,     0x08($s7)            # TCON = 3
			8'h62:    Instruction <= 32'haeeb0008;
				//addi    $t4,     $t4,          4
			8'h63:    Instruction <= 32'h218c0004;
				//andi    $t4,     $t4,          0x0c   
			8'h64:    Instruction <= 32'h318c000c;
				//sw      $t4,     0x50($zero) 
			8'h65:    Instruction <= 32'hac0c0050;
				//jr      $k0
			8'h66:    Instruction <= 32'h03400008;
				//j       Exception
			8'h67:    Instruction <= 32'h08000067;
