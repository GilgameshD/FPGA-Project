...
float** cov_layer3_6(float** map, nSize mapSize, float** inputData, nSize inSize, int type) // 卷积操作
{
	float* block_to_row_stream = (float*)malloc(8*8*5*5*sizeof(float));
...
//	    res2[i] = (float*)malloc((inSize.c - mapSize.c + 1)*sizeof(float));
	...
	matrix_multiple_hw(block_to_row_stream, map, res, mapSize, inSize);
	int cov2layerend = XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR, TIMER_COUNTER_0);
	xil_printf("%d cycles spent on cov2\n", cov2layerend - cov2layerstart);
	//matrix_multiple(block_to_row_stream, flipmap, res2, mapSize, inSize);
...
float** cov_layer1_6(float** flipmap, nSize mapSize, float** inputData, nSize inSize, int type) // 卷积操作
{
	int i,j,k,l;
...
//	    res2[i] = (float*)malloc((inSize.c - mapSize.c + 1)*sizeof(float));
    ...
    for (i = 0; i < 9; i++){
    	...
    	//Setup_HW_Accelerator(block_to_row_stream[i], map, out[i], 1024);
    	//Run_HW_Accelerator(block_to_row_stream[i], map, out[i], 1024);
    }
	int cov2layerend = XTmrCtr_GetTimerCounterReg(XPAR_TMRCTR_0_BASEADDR, TIMER_COUNTER_0);
	//xil_printf("%d cycles spent on cov2\n", cov2layerend - cov2layerstart);
	//matrix_multiple(block_to_row_stream, flipmap, res2, mapSize, inSize);
