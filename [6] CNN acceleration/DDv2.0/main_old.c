...
////	int res_sw[DIM][DIM];
////
////	unsigned int dma_size = SIZE * sizeof(int);
////
////    int acc_factor;
...
////		acc_factor = (int) run_time_sw / (int) run_time_hw;
...
//	//int res_hw[DIM*DIM];
//	//int res_sw[DIM*DIM];
//	float input_fm[INPUT_FM_SIZE];
//	float weights[WEIGHTS_SIZE];
//	float output_fm_sw[PATTERN_ROW*BLOCK_TO_ROW_ROW];
//	float output_fm_hw[PATTERN_ROW*BLOCK_TO_ROW_ROW];
////		int A[DIM*DIM];
////		int B[DIM*DIM];
//
//	unsigned int dma_size = SIZE * sizeof(int);
...
//	xil_printf("\rcalibration: %d cycles.\r\n", calibration);
//
//	for (i = 0; i < BLOCK_TO_ROW_ROW; i++){
//		for (j = 0; j < PATTERN_COL; j++){
//			input_fm[i*PATTERN_COL+j] = 0.18739*i;
//		}
//	}
//
...
//		for (j = 0; j < BLOCK_TO_ROW_ROW; j++)
//		output_fm_sw[i*BLOCK_TO_ROW_ROW + j] = 0;
//		output_fm_hw[i*BLOCK_TO_ROW_ROW + j] = 0;
//	}
//
//
//
...
//#endif
//
//		matrix_multiply_ref(input_fm, weights, output_fm_sw);
//
//#ifdef	USE_HIGH_LEVEL_API_TIMERS
//		end_time = XTmrCtr_GetValue(&timer_dev, XPAR_AXI_TIMER_0_DEVICE_ID);
//#else
...
//				run_time_sw);
//
////	    Xil_DCacheFlushRange((unsigned int)input_fm,INPUT_FM_SIZE*sizeof(int));
////        Xil_DCacheFlushRange((unsigned int)weights,WEIGHTS_SIZE*sizeof(int));
////		Xil_DCacheFlushRange((unsigned int)output_fm_hw,OUTPUT_FM_SIZE*sizeof(int));
//
//		// Run the HW Accelerator
//		Setup_HW_Accelerator(input_fm, weights, output_fm_hw, dma_size);
//
//#ifdef	USE_HIGH_LEVEL_API_TIMERS
//		XTmrCtr_Reset(&timer_dev, XPAR_AXI_TIMER_0_DEVICE_ID);
//		begin_time = XTmrCtr_GetValue(&timer_dev, XPAR_AXI_TIMER_0_DEVICE_ID);
...
//#endif
//
//		Run_HW_Accelerator(input_fm, weights, output_fm_hw, dma_size);
//
//#ifdef	USE_HIGH_LEVEL_API_TIMERS
//		end_time = XTmrCtr_GetValue(&timer_dev, XPAR_AXI_TIMER_0_DEVICE_ID);
//#else
...
//
//		//Compare the results from sw and hw
//		for (i = 0; i < OUTPUT_FM_SIZE; i++){
//			if (output_fm_sw[i] != output_fm_hw[i])
//				err++;
//			//xil_printf("%d ", output_fm_sw[i]);
//		}
////		for (i = 0; i < DIM; i++)
////			for (j = 0; j < DIM; j++)
////				if (res_sw[i*DIM+j] == res_hw[i*DIM+j]) {
////					xil_printf("%d,%d ",res_sw[i*DIM+j],res_hw[i*DIM+j]);
////				}
////
//		if (err == 0)
//			print("\rSW and HW results match!\n\r");
